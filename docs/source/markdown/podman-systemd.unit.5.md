...
Alternatively, you can use a single generic volume (or network) template and
reference specific instances by name. This avoids creating a separate template
file per volume role (config, data, cache, and so on):
...
```
[Container]
Volume=volume@app-config.volume:/config
Volume=volume@%N-data.volume:/data
```

Quadlet resolves `volume@app-config.volume` against the template unit
`volume@.volume` and generates a dependency on the systemd instance
`volume-volume@app-config.service`. The volume name is taken from the template
(`VolumeName=`, or the default `systemd-…-%i` form) with `%i` replaced by the
instance name. There is no need to create an on-disk `volume@app-config.volume`
file; systemd instantiates the template service.

When the consuming unit is not itself a template, the `%N` specifier in the
instance string is expanded to the unit's service name at generation time
(for example `volume@%N-data.volume` on `app.container` becomes
`volume@app-data.volume`). The instance name must be a valid systemd instance
name.

The same instance form is supported for `Network=` (`.network` templates) and
for `Mount=` sources that refer to `.volume` units.
